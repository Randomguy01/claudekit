...
# androidx.datastore.preferences.rxjava3 — Top-Level Functions

The package-level helper in `androidx.datastore.preferences.rxjava3`.
...
Creates a property delegate for a single-process Preferences [`RxDataStore`](../androidx.datastore.rxjava3/rx-data-store.md). Call this only once per file (at the top level); all usages of the DataStore should reference the same instance. The receiver type for the property delegate must be a `Context`. Use only from a single application in a single classloader in a single process.
