...
WorkManager has built-in support for setting and observing intermediate progress for workers. If the worker runs while the app is in the foreground, this information can be shown to the user using APIs that return the [`LiveData`](https://developer.android.com/reference/androidx/lifecycle/LiveData) of [`WorkInfo`](../api/androidx.work/work-info.md).
