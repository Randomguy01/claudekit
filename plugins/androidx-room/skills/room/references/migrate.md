...
If Room detects ambiguous schema changes and it can't generate a migration plan without more input, it throws a compile-time error. Then, you must implement an [`AutoMigrationSpec`](../api/auto-migration-spec.md). This occurs when a migration involves one of the following:
...
- [`@DeleteTable`](../api/delete-table.md)
- [`@RenameTable`](../api/rename-table.md)
- [`@DeleteColumn`](../api/delete-column.md)
- [`@RenameColumn`](../api/rename-column.md)
...
If you need to do more work after the automated migration completes, implement [`onPostMigrate()`](../api/auto-migration-spec.md). If you implement this method in your `AutoMigrationSpec`, Room calls it after the automated migration completes.

**In Kotlin, if you have multiple migrations of the same type you must use a container for multiple annotations, such as [`@RenameTable.Entries`](../api/rename-table.md)**

## Manual Migrations

For complex schema changes, Room might not be able to generate a migration path automatically. If you decide to split the data in a table into two tables, Room can't tell how to perform this split. In cases like these, you must manually define a migration path by implementing a [`Migration`](../api/migration.md) class.

A `Migration` class explicitly defines a migration path between a `startVersion` and an `endVersion` by overriding the [`Migration.migrate()`](../api/migration.md) method. Add the `Migration` classes to the database builder using the [`addMigrations()`](../api/database.md) method:
...
The testing package provides a [`MigrationTestHelper`](../api/migration-test-helper.md) class, which can read exported schema files. The package also implements the JUnit4 [`TestRule`](../api/test-rule.md) interface, so it can manage created databases.
...
In Room 2.2.0 and higher, you can define a default value for a column by using the annotation [`@ColumnInfo(defaultValue = "...")`](../api/column-info.md). In versions lower than 2.2.0, the only way to define a default value for a column is by defining it directly in an executed SQL statement, which creates a default value that Room does not know about. This means that if a database is originally created by a version of Room lower than 2.2.0, upgrading your app to use Room 2.2.0 might require you to provide a special migration path for existing default values that you defined without using Room APIs.
